...
2. ダウンロードした `.exe` ファイルを任意の場所に置く
3. `.exe` をダブルクリックして起動
4. アプリが起動すればインストール完了（インストーラー不要）

> [!NOTE]
> アプリを起動すると、`.exe` と同じフォルダーにデータベースファイル (`data.db`) が生成されます

## アップデート方法

1. アプリが起動中の場合は終了する
2. [GitHub Releases](https://github.com/noma-t/ValoAccounts/releases) から最新版の `.exe` ファイルをダウンロード
3. 古い `.exe` ファイルを削除し、新しい `.exe` に置き換える
4. 新しい `.exe` を起動する

> [!NOTE]
> `data.db` はそのまま残してください。アカウント情報はデータベースに保存されているため、`.exe` を入れ替えてもデータは引き継がれます
...
- **ショップ表示**: アカウントごとに本日のデイリーショップ・バンドル・ナイトマーケットを別ウィンドウで確認
...
- ショップ機能は、対象アカウントで Riot Client にログイン済みの状態でのみ利用できます
